...
VGG16 , preprocess_input = Classifiers_3D.get('vgg16') # 3D VGG16
...
def create_model(input_shape, num_classes):
    ...
    model = Model(inputs = conv_base.input, outputs = output_layer, name = '3D_VGG16')
    
    return model 

# Build model.
vgg16_model = create_model((128,128,64,3), 2)
vgg16_model.summary()
...
checkpoint_cb = keras.callbacks.ModelCheckpoint("3D_ALZ_VGG16_v3.h5", save_best_only = True, verbose = 1)
...
csv_log = keras.callbacks.CSVLogger("VGG16_history_{}_v3".format('classification'), append = True)
...
r    unning_secs = (dt.now() - start).seconds
