...
ResNet50 , preprocess_input = Classifiers_3D.get('resnet50') # 3D ResNet50
...
def create_model(input_shape, num_classes):
    ...
    output_layer = Dense(num_classes, activation = 'sigmoid')(top_model)
    
    
    # Group all layers into a Model() object
    
    model = Model(inputs = conv_base.input, outputs = output_layer, name = '3D_ResNet50')
    
    return model 

# Build model.
resnet50_model = create_model((128,128,64,3), 2)
...
checkpoint_cb = keras.callbacks.ModelCheckpoint("3D_ALZ_ResNet50_v2.h5", save_best_only = True, verbose = 1)
...
csv_log = keras.callbacks.CSVLogger("ResNet50_history_{}_v2".format('classification'), append = True)
...
model_history = resnet50_model.fit(
    train_generator,
